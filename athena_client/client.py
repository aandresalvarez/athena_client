...
from .exceptions import AthenaError, APIError
...
class AthenaClient:
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        Args:
            ...
            retry_delay: Delay between retry attempts in seconds (overrides backoff_factor)
    ...
    def search(
        ...
        **kwargs: Any,
        ...
        max_attempts = max(1, max_retries if max_retries is not None else (self.max_retries if auto_retry else 1))
        retry_delay_seconds = retry_delay if retry_delay is not None else self.retry_delay
        ...
        for attempt in range(max_attempts):
            try:
                ...
                if isinstance(response, dict) and response.get("result") is None and "errorMessage" in response and "errorCode" in response:
                    error_msg = response.get("errorMessage", "Unknown API error")
                    error_code = response.get("errorCode")
                    raise APIError(f"Search failed: {error_msg}", api_error_code=error_code, api_message=error_msg)
                
                # Existing special cases (now redundant, but kept for clarity)
                if isinstance(response, dict) and response.get("result") is None and "errorMessage" in response:
                    error_msg = response.get("errorMessage", "Unknown API error")
                    error_code = response.get("errorCode")
                    
                    if "Page size must not be less than one" in error_msg:
                        raise APIError(
                            f"Invalid page size: {error_msg}. Please use a page size of 1 or greater.",
                            api_error_code=error_code,
                            api_message=error_msg
                        )
                    elif "Page size must not be greater than" in error_msg:
                        raise APIError(
                            f"Page size too large: {error_msg}. Please reduce the page size.",
                            api_error_code=error_code,
                            api_message=error_msg
                        )
                    elif "Query must not be blank" in error_msg:
                        raise APIError(
                            f"Empty search query: {error_msg}. Please provide a search term.",
                            api_error_code=error_code,
                            api_message=error_msg
                        )
                    else:
                ...
                if isinstance(e, APIError):
                    ...
                    # Log retry attempt
                    logger.info(f"Retrying search due to {type(e).__name__} (attempt {attempt + 1}/{max_attempts}): {e}")
                    ...
                    # Final attempt failed, raise with detailed retry information
            ...
            try:
                ...
                if isinstance(response, dict) and response.get("result") is None and "errorMessage" in response:
                    ...
                    if "Unable to find" in error_msg and "ConceptV5" in error_msg:
                        raise APIError(
                            f"Concept not found: Concept ID {concept_id} does not exist in the database. "
                            f"Please verify the concept ID is correct.",
                            api_error_code=error_code,
                            api_message=error_msg
                        )
                    else:
                        raise APIError(f"Failed to get concept details: {error_msg}", api_error_code=error_code, api_message=error_msg)
                ...
                if isinstance(e, APIError):
                    ...
                    # For other errors, retry if we have attempts left
                    logger.info(f"Retrying concept details due to error (attempt {attempt + 1}/{max_attempts}): {e}")
                    continue
                else:
    ...
    def relationships(self, concept_id: int, auto_retry: bool = True) -> ConceptRelationship:
        """
            ...
            try:
                ...
                if isinstance(response, dict) and response.get("result") is None and "errorMessage" in response:
                    ...
                    if "Unable to find" in error_msg and "ConceptV5" in error_msg:
                        ...
                        raise APIError(f"Failed to get relationships: {error_msg}", api_error_code=error_code, api_message=error_msg)
                ...
                if isinstance(e, APIError):
                    ...
                    # For other errors, retry if we have attempts left
                    logger.info(f"Retrying relationships due to error (attempt {attempt + 1}/{max_attempts}): {e}")
                    continue
                else:
                    # Final attempt failed, raise with enhanced message
                    raise AthenaError(
    ...
    def graph(
        ...
        **kwargs: Any,
        ...
        for attempt in range(max_attempts):
            try:
                response = self.http.get(f"/concepts/{concept_id}/relations", params=params)
                ...
                if isinstance(response, dict) and response.get("result") is None and "errorMessage" in response:
                    ...
                    if "Unable to find" in error_msg and "ConceptV5" in error_msg:
                        ...
                        raise APIError(f"Failed to get concept graph: {error_msg}", api_error_code=error_code, api_message=error_msg)
                ...
                if isinstance(e, APIError):
                    ...
                    # For other errors, retry if we have attempts left
                    logger.info(f"Retrying graph due to error (attempt {attempt + 1}/{max_attempts}): {e}")
                    continue
                else:
                    # Final attempt failed, raise with enhanced message
                    raise AthenaError(
